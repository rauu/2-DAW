...
function almacenar_tabla() {
    //Meter los datos en localhost
    let dni = " ";
    let ref = " ";
    let precio = " ";
    let cantidad = " ";
    ...
    let array_json = [];
    for (let x = 0; x < document.getElementsByClassName("rows").length; x++) {
        console.log(document.getElementById("lineas").rows[x + 1].cells.length);
        dni = document.getElementById("lineas").rows[x + 1].cells[0].innerText;
        ref = document.getElementById("lineas").rows[x + 1].cells[1].innerText;
        precio = document.getElementById("lineas").rows[x + 1].cells[2].innerText;
        cantidad = document.getElementById("lineas").rows[x + 1].cells[3].innerText;
        total = document.getElementById("lineas").rows[x + 1].cells[4].innerText;
        boton = document.getElementById("lineas").rows[x + 1].cells[5].innerText;
    ...
    localStorage.setItem('tabla', JSON.stringify(array_json));
...
function recuperar_tabla() {
    ...
    let total = " ";
    let boton = " ";
    let recuperar_array = JSON.parse(localStorage.getItem("tabla"));


    console.log(recuperar_array);
    for (let i = 0; i < recuperar_array.length; i++) {

        dni = recuperar_array[i].dni;
        ref = recuperar_array[i].ref;
        precio = recuperar_array[i].precio;
        cantidad = recuperar_array[i].cantidad;
        total = recuperar_array[i].total;
        boton = recuperar_array[i].boton;
        ...
        //creacion de Boton
        let boton_table = document.createElement("button");
        boton_table.innerText = boton;
        td_borrar.appendChild(boton_table);

        //Agregacion de los clases si algun elemento lo necesite
        boton_table.classList.add("boton");
        td_total.classList.add("total_num", "a_derecha");
        td_precio.classList.add("a_derecha");
        td_cantidad.classList.add("a_derecha");
        tr.classList.add("rows");
        ...
        //Asignacion de onlick a boton Borrar
        //boton.setAttribute('onclick', Borrar);
        boton_table.addEventListener('click', () => {
            tr.remove();
        ...
        //Meter los valores a la tabla pero los valores que se recogen con JSOM
        td_dni.innerText = dni;
        td_ref.innerText = ref;
        td_precio.innerText = precio;
        td_cantidad.innerText = cantidad;
        td_total.innerText = precio;
...
function Precio() {
    ...
    var total = document.getElementsByClassName("total_num");
    var sum = 0;
    ...
    var total_html = document.getElementsByClassName("total")[0];
